...
type ScraperService interface {
	GetSalesByLocation(city, state string) ([]listing.ScrapedListing, error)
}

// NewSaleHandler creates a new sale handler
func NewSaleHandler(saleService *listing.Service, userService *user.Service) *SaleHandler {
	return &SaleHandler{
...
func (h *SaleHandler) SetScraperService(scraper ScraperService) {
...
func (h *SaleHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Create the sale
	if err := h.saleService.CreateSale(&s); err != nil {
		api.InternalErrorResponse(w, fmt.Sprintf("Failed to create sale: %v", err))
		return
	}
...
func (h *SaleHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	s, err := h.saleService.GetSaleByID(id)
	if err != nil {
...
func (h *SaleHandler) GetAll(w http.ResponseWriter, r *http.Request) {
	...
	sales, err := h.saleService.GetAllSales(filters)
	if err != nil {
...
func (h *SaleHandler) GetMySales(w http.ResponseWriter, r *http.Request) {
	...
	sales, err := h.saleService.GetSellerSales(u.ID)
	if err != nil {
...
func (h *SaleHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	existingSale, err := h.saleService.GetSaleByID(id)
	if err != nil {
	...
	if existingSale.SellerID == nil || *existingSale.SellerID != u.ID {
	...
	if err := h.saleService.UpdateSale(&s); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *SaleHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.saleService.DeleteSale(id); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *SaleHandler) AddImage(w http.ResponseWriter, r *http.Request) {
	// Get user ID from context
	uid := r.Context().Value(middleware.ContextKeyUID).(string)
	u, err := h.userService.GetOrCreateUser(uid, "")
	if err != nil {
	...
	saleID, err := strconv.Atoi(pathParts[3])
	if err != nil {
		http.Error(w, "Invalid sale ID", http.StatusBadRequest)
		return
	}
	...
	// Verify ownership
	existingSale, err := h.saleService.GetSaleByID(saleID)
	if err != nil {
		http.Error(w, "Sale not found", http.StatusNotFound)
		return
	}
	...
	if err := h.saleService.AddSaleImage(&img); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *SaleHandler) GetAggregatedSales(w http.ResponseWriter, r *http.Request) {
	// Parse query parameters
	query := r.URL.Query()
	city := query.Get("city")
	state := query.Get("state")
	...
	var aggregatedSales []*listing.AggregatedListing
	...
	ownedSales, err := h.saleService.GetAllSales(filters)
	if err != nil {
	...
	sort.Slice(aggregatedSales, func(i, j int) bool {
		return aggregatedSales[i].StartDate.After(aggregatedSales[j].StartDate)
	})

	api.OKResponse(w, map[string]interface{}{
		"sales": aggregatedSales,
		"total": len(aggregatedSales),
	}, "")
}
