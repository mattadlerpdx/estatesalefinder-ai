...
// SaleRepository implements the listing.Repository interface
...
func (r *SaleRepository) Create(s *listing.Listing) error {
	query := `
		INSERT INTO listings (
			listing_type, seller_id, title, description, sale_type, status,
			address_line1, address_line2, city, state, zip_code, latitude, longitude,
			start_date, end_date, sale_hours,
	...
	err := r.db.QueryRow(
		query,
		s.ListingType, s.SellerID, s.Title, s.Description, s.SaleType, s.Status,
		s.AddressLine1, s.AddressLine2, s.City, s.State, s.ZipCode, s.Latitude, s.Longitude,
		s.StartDate, s.EndDate, s.SaleHours,
...
func (r *SaleRepository) GetByID(id int) (*listing.Listing, error) {
	query := `
		SELECT id, seller_id, title, description, sale_type, status,
			address_line1, address_line2, city, state, zip_code, latitude, longitude,
			start_date, end_date, sale_hours,
			listing_tier, payment_status, amount_paid,
			view_count, featured, created_at, updated_at
	...
	err := r.db.QueryRow(query, id).Scan(
		&s.ID, &s.SellerID, &s.Title, &s.Description, &s.SaleType, &s.Status,
		&s.AddressLine1, &s.AddressLine2, &s.City, &s.State, &s.ZipCode, &s.Latitude, &s.Longitude,
		&s.StartDate, &s.EndDate, &s.SaleHours,
		&s.ListingTier, &s.PaymentStatus, &s.AmountPaid,
		&s.ViewCount, &s.Featured, &s.CreatedAt, &s.UpdatedAt,
...
func (r *SaleRepository) GetAll(filters listing.ListingFilters) ([]listing.Listing, error) {
	query := `
		SELECT id, seller_id, title, description, sale_type, status,
			address_line1, address_line2, city, state, zip_code, latitude, longitude,
			start_date, end_date, sale_hours,
	...
	if filters.SaleType != "" {
		query += fmt.Sprintf(" AND sale_type = $%d", argPos)
		args = append(args, filters.SaleType)
		argPos++
	}
	if filters.Status != "" {
...
func (r *SaleRepository) GetBySellerID(sellerID int) ([]listing.Listing, error) {
	query := `
		SELECT id, seller_id, title, description, sale_type, status,
			address_line1, address_line2, city, state, zip_code, latitude, longitude,
			start_date, end_date, sale_hours,
...
func (r *SaleRepository) Update(s *listing.Listing) error {
	query := `
		UPDATE listings SET
			title = $1, description = $2, sale_type = $3, status = $4,
			address_line1 = $5, address_line2 = $6, city = $7, state = $8, zip_code = $9,
			latitude = $10, longitude = $11,
			start_date = $12, end_date = $13, sale_hours = $14,
	...
	result, err := r.db.Exec(
		query,
		s.Title, s.Description, s.SaleType, s.Status,
...
func (r *SaleRepository) IncrementViewCount(id int) error {
	query := `UPDATE listings SET view_count = view_count + 1 WHERE id = $1`
	_, err := r.db.Exec(query, id)
	return err
}

// AddImage adds an image to a sale
func (r *SaleRepository) AddImage(img *listing.ListingImage) error {
	query := `
		INSERT INTO sale_images (sale_id, image_url, thumbnail_url, is_primary, display_order, uploaded_at)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id
	`
	...
	err := r.db.QueryRow(
		query,
		img.SaleID, img.ImageURL, img.ThumbnailURL, img.IsPrimary, img.DisplayOrder, img.UploadedAt,
	).Scan(&img.ID)
...
// GetImagesBySaleID retrieves all images for a sale
func (r *SaleRepository) GetImagesBySaleID(saleID int) ([]listing.ListingImage, error) {
	query := `
		SELECT id, sale_id, image_url, thumbnail_url, is_primary, display_order, uploaded_at
		FROM sale_images
		WHERE sale_id = $1
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&img.ID, &img.SaleID, &img.ImageURL, &img.ThumbnailURL,
...
func (r *SaleRepository) DeleteImage(imageID int) error {
	query := `DELETE FROM sale_images WHERE id = $1`
...
// SetPrimaryImage sets an image as primary and unsets all others for that sale
func (r *SaleRepository) SetPrimaryImage(imageID int, saleID int) error {
	// Start a transaction
	tx, err := r.db.Begin()
	if err != nil {
	...
	_, err = tx.Exec(`UPDATE sale_images SET is_primary = false WHERE sale_id = $1`, saleID)
	if err != nil {
	...
	result, err := tx.Exec(
		`UPDATE sale_images SET is_primary = true WHERE id = $1 AND sale_id = $2`,
		imageID, saleID,
	)
	if err != nil {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("image not found or doesn't belong to this sale")
	}
...
func (r *SaleRepository) UpsertExternalSale(s *listing.Listing) error {
	query := `
		INSERT INTO listings (
	...
	if err != nil {
		return fmt.Errorf("failed to upsert external sale: %w", err)
	}
...
func (r *SaleRepository) GetExternalSalesByLocation(city, state string) ([]listing.Listing, error) {
	query := `
...
func (r *SaleRepository) GetLastScrapedTime(city, state string) (*listing.Listing, error) {
	query := `
